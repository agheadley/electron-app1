...
<v-btn color="red" text @click="deleteAll">Delete All</v-btn>
...
export default {
  ...
  created() {
    ...
    if(this.$store.state.timetableYears.length>0) this.timetableYears=this.$store.state.timetableYears;
    else this.timetableYears=settings.schoolYears;
  ...
  methods: {
    ...
    deleteAll() {
      ...
      this.isBlocks=false;
      this.isBlocksValidated=false;
      this.isTimetableYearsValid=false;
